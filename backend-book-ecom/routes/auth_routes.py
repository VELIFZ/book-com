...
def register():
    try:
        ...
        name = data.get('name') or data.get('firstName') or data.get('first_name')
        last_name = data.get('last_name') or data.get('lastName') or data.get('lastname')
        email = data.get('email')
        phone_number = data.get('phone_number') or data.get('phoneNumber') or '000-000-0000'
        ...
        existing_query = select(users_table).where(users_table.c.email == email)
        existing_user = execute_query(existing_query, single_result=True)
        
        if existing_user:
            return jsonify({'message': 'Email already registered'}), 409
        
        print(f"Creating user with: name={name}, last_name={last_name}, email={email}, phone={phone_number}")
